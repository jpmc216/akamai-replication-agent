...
public class AkamaiEdgeGridSignerImpl implements AkamaiEdgeGridSigner {

	/**
	 * The logger used for logging.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(AkamaiEdgeGridSignerImpl.class);
	 ...
	 */
	public AkamaiEdgeGridSignerImpl(List<String> headers, int maxBodySize) {
	 ...
	 */
	private static byte[] getHash(byte[] contentBytes, int offset, int len) throws AkamaiException {
		try {
			...
			throw new AkamaiException("Failed to get request hash: algorithm not found", nsae);
		}
	}
	 ...
	 * @throws AkamaiException
	 */
	protected CanonicalizerHelper getCanonicalizedRequest(HttpRequestBase request) throws AkamaiException {
		StringBuilder sb = new StringBuilder();

		String method = request.getMethod();
		if (StringUtils.isEmpty(method)) {
			throw new AkamaiException("Invalid request: empty request method");
		}
		...
		if (StringUtils.isEmpty(scheme)) {
			throw new AkamaiException("Invalid request: empty request scheme");
		}
		...
		if (StringUtils.isEmpty(host)) {
			throw new AkamaiException("Invalid request: empty host");
		}
	 ...
	 * @throws AkamaiException
	 */
	protected CanonicalizerHelper getContentHash(HttpRequestBase request) throws AkamaiException {
		String data = "";
		...
		if ("POST".equalsIgnoreCase(request.getMethod())) {
			...
			if (httpContent != null) {
				try {
					...
					throw new AkamaiException("Failed to get content hash: failed to read content", ioe);
				}
			}
		}
	...
	public HttpRequestBase sign(HttpRequestBase request, ClientCredential credential) throws AkamaiException {
		...
		try {
			...
			throw new AkamaiException("Failed to sign: invalid string encoding", uee);
		}
	}
	 ...
	 * @throws AkamaiException
	 */
	private static byte[] sign(String s, byte[] key, String algorithm) throws AkamaiException {
		try {
			...
			throw new AkamaiException("Failed to sign: algorithm not found", nsae);
		} catch (InvalidKeyException ike) {
			throw new AkamaiException("Failed to sign: invalid key", ike);
		} catch (UnsupportedEncodingException uee) {
			throw new AkamaiException("Failed to sign: invalid string encoding", uee);
		}
	}
